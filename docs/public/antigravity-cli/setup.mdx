...
description: "Add persistent memory to Antigravity CLI with memsmith"
---
...
4. Register memsmith's MCP server in `~/.gemini/antigravity/mcp_config.json` **and** `~/.gemini/config/mcp_config.json`
5. Write a rules/context placeholder to `~/.agents/rules/memsmith-context.md`
...
<Tabs>
  <Tab title="Gemini API (Free)">
    ...
    cat > ~/.memsmith/settings.json << 'EOF'
    {
      ...
      "MEMSMITH_GEMINI_API_KEY": "YOUR_API_KEY"
    }
    EOF
    ```
  ...
  <Tab title="OpenRouter">
    ...
    cat > ~/.memsmith/settings.json << 'EOF'
    {
      ...
      "MEMSMITH_OPENROUTER_API_KEY": "YOUR_KEY"
    }
    EOF
    ```
  </Tab>
...
# Check hooks are installed — look for memsmith entries
cat ~/.gemini/settings.json | grep memsmith

# Check MCP registration in either candidate config path
cat ~/.gemini/antigravity/mcp_config.json | grep memsmith
cat ~/.gemini/config/mcp_config.json | grep memsmith
```
...
On session start, you'll see memsmith context injected with your recent observations and project history.
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with memsmith commands.
...
3. Check your AI provider is configured in `~/.memsmith/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your memsmith version may need updating — the Antigravity CLI adapter strips ANSI color codes automatically:
...
```bash
npx memsmith uninstall
```

This removes memsmith's hooks from `~/.gemini/settings.json`, cleans up the context section in `~/.gemini/GEMINI.md`, removes memsmith's entry from both MCP config files, and removes the rules context section — while preserving everything else in those files.
