...
|---------|--------|---------|-------------|
| `MEMSMITH_PROVIDER` | `claude`, `gemini`, `openrouter` | `claude` | AI provider for observation extraction |
| `MEMSMITH_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `MEMSMITH_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `MEMSMITH_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `MEMSMITH_OPENROUTER_APP_NAME` | string | `memsmith` | Optional: App name for analytics |
...
```json
{
  ...
  "MEMSMITH_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
...
The full conversation history is sent to the model on each request. memsmith does
...
Either:
- Set `MEMSMITH_OPENROUTER_API_KEY` in `~/.memsmith/settings.json`, or
...
HTTP-Referer: https://github.com/shshalom/memsmith
X-Title: memsmith
