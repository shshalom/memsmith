...
Knowledge agents let you compile a slice of your memsmith observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.
...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="memsmith" limit=200
```
...
Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.memsmith/corpora/hooks-expertise.corpus.json`.
...
- **Model from settings**: Reads `MEMSMITH_MODEL` from user settings -- no hardcoded model IDs
