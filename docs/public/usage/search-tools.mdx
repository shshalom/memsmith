...
Instead of fetching all historical data upfront (expensive), memsmith uses a progressive disclosure approach:
...
**MCP Server:** Located at `~/.claude/plugins/marketplaces/shshalom/plugin/scripts/mcp-server.cjs`

**Worker Service:** Express API on port 37777, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.memsmith/memsmith.db`
