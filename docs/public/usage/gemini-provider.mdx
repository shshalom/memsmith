...
|---------|--------|---------|-------------|
| `MEMSMITH_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `MEMSMITH_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `MEMSMITH_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` | `gemini-2.5-flash-lite` | Gemini model to use |
| `MEMSMITH_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "MEMSMITH_GEMINI_API_KEY": "your-api-key-here",
  "MEMSMITH_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "MEMSMITH_GEMINI_BILLING_ENABLED": "true"
}
```
...
If Gemini is selected and the API errors, memsmith logs the failure and re-throws so the message stays pending for later retry. There is no Claude SDK fallback — earlier docs claimed automatic Claude fallback, but the wiring was never actually engaged in production (#2087). To switch providers, change `MEMSMITH_PROVIDER` in settings.
...
Either:
- Set `MEMSMITH_GEMINI_API_KEY` in `~/.memsmith/settings.json`, or
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows memsmith to process observations instantly instead of waiting between requests.
</Tip>
