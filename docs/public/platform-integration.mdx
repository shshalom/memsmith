...
description: Complete reference for integrating memsmith worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building memsmith integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// or whatever the user set via MEMSMITH_WORKER_PORT / settings.json. Read it from
// process.env.MEMSMITH_WORKER_PORT, then ~/.memsmith/settings.json
// (MEMSMITH_WORKER_PORT key), then fall back to the deterministic default.
...
```bash
MEMSMITH_MODEL=claude-haiku-4-5-20251001  # Default Claude model for observations/summaries
MEMSMITH_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
MEMSMITH_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
MEMSMITH_DATA_DIR=                        # Optional override for the data directory
...
**Tag Stripping:** Removes `<private>` and `<memsmith-context>` tags before storage.
...
```json Response
{
  "projects": ["memsmith", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('memsmith-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('memsmith', async (request, context, stream, token) => {
...
```json
{
  "name": "memsmith-vscode",
  "displayName": "MemSmith",
  "version": "1.0.0",
  "engines": {
    "vscode": "^1.95.0"
  },
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, MEMSMITH_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('memsmith-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://memsmith.ai">
    Complete memsmith documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/shshalom/memsmith">
