...
description: "Common issues and solutions for MemSmith"
---
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `MEMSMITH_WORKER_PORT` in `~/.memsmith/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .MEMSMITH_WORKER_PORT ~/.memsmith/settings.json)
   lsof -i :$PORT
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('memsmith-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('memsmith-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/shshalom/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/shshalom/.install-version
   cat ~/.claude/plugins/marketplaces/shshalom/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.memsmith/memsmith.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/shshalom/package.json
   ```
...
2. Backup database:
   ```bash
   cp ~/.memsmith/memsmith.db ~/.memsmith/memsmith.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.memsmith/memsmith.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install memsmith` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/shshalom/
   ```
...
jq -r .MEMSMITH_WORKER_PORT ~/.memsmith/settings.json
...
**Solution**: Check `~/.memsmith/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/shshalom/memsmith/issues)
...
- [Configuration](configuration) - Customize MemSmith
