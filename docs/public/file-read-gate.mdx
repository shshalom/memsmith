...
description: "How memsmith intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `MEMSMITH_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.memsmith/settings.json`:

```json
{
  "MEMSMITH_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
2. Find the memsmith hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of memsmith's layered context strategy:
