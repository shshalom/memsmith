...
description: "How memsmith evolved from v3 to v5+"
---
...
This is the story of how memsmith evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('memsmith-theme') || 'system';
  });

  useEffect(() => {
    localStorage.setItem('memsmith-theme', theme);
  }, [theme]);
...
**Impact**: Cross-platform compatibility restored, Windows users can now use memsmith without issues.
...
```bash
cp ~/.memsmith/memsmith.db ~/.memsmith/memsmith-v3-backup.db
```
