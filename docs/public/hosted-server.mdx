...
description: "Remote authenticated MCP recall, usage metering + quotas, and data deletion — how memsmith's cloud server works today."
---
...
The hosted server is the cloud side of memsmith: a Postgres-backed HTTP service
(`/v1`) plus a separate BullMQ generation worker. Where the local plugin keeps
memory in `~/.memsmith/memsmith.db` on your machine, the hosted server keeps
...
```
 ...
 │  - auth (api-key mode)       ├───────▶│  memsmith server         │
...
Set `MEMSMITH_AUTH_MODE=api-key` and send `Authorization: Bearer <key>` on every
...
- **`GET /v1/connect`** (requires `memories:read`) returns the same command with a
  ...
  `MEMSMITH_PUBLIC_URL` (recommended when behind a proxy or load balancer) or,
...
|---------|--------|------------------------|
| `MEMSMITH_RATE_LIMIT_PER_MIN` | Max requests per **API key** per minute. | `429` with `Retry-After` and `X-RateLimit-*` headers. |
| `MEMSMITH_MONTHLY_REQUEST_CAP` | Max requests per **team** per calendar month (UTC). | `402 quota_exceeded`. |
| `MEMSMITH_MONTHLY_TOKEN_CAP` | Max provider **tokens** per team per month. Gates **writes only** — reads stay open so a team over budget can still recall. | `402` at the cap. |
| `MEMSMITH_USAGE_METERING=1` | Records one `request` usage event per authenticated call (fire-and-forget). | — |
...
the separate BullMQ worker (`memsmith server worker start`) — the HTTP path
