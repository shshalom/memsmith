...
description: "System components and data flow in MemSmith"
---
...
MemSmith operates as a Claude Code plugin with the following core components:
...
│    "run: npx memsmith repair" to stderr. Always exits 0.      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
The actual runtime install (Bun, uv, `bun install`) is performed by `npx memsmith install` / `npx memsmith repair` with a visible installer spinner; the Setup hook itself only reads the `.install-version` marker.

See [Plugin Hooks](/architecture/hooks) for detailed hook documentation.

### 2. Worker Service
Express.js HTTP server on a per-user port (default `37700 + (uid % 100)`, override via `MEMSMITH_WORKER_PORT`) with:
