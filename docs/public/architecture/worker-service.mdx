...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `MEMSMITH_WORKER_PORT`). The active port is stored in `~/.memsmith/settings.json` and reported by `GET /api/health`.
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `MEMSMITH_MODEL` (default: `claude-haiku-4-5-20251001`)
...
The `port` value is the actual worker port for the current user — per-user default `37700 + (uid % 100)`, or whatever `MEMSMITH_WORKER_PORT` is set to. The example above is illustrative; your value will differ.
...
- PID file tracking at `~/.memsmith/worker.pid`
...
Bun is required to run the worker service. If Bun is not installed, `npx memsmith install` (and `npx memsmith repair`) installs it globally during setup, with a visible clack spinner:
...
Set the Claude model used for compression via environment variable or `~/.memsmith/settings.json`:

```bash
export MEMSMITH_MODEL=claude-haiku-4-5-20251001
```
...
- **Override**: Set `MEMSMITH_WORKER_PORT` (env or `~/.memsmith/settings.json`)
- **Discovery**: `GET /api/health` returns the active port; `~/.memsmith/settings.json` stores the configured value

If the chosen port is occupied, the worker fails to start — pin a different port via `MEMSMITH_WORKER_PORT` and restart.
...
```
~/.memsmith/
├── memsmith.db           # SQLite database (bun:sqlite)
