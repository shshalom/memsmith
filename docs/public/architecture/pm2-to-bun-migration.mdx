...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing memsmith for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
- **Logs**: `~/.pm2/logs/memsmith-worker-*.log`
- **PID File**: `~/.pm2/pids/memsmith-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop memsmith-worker   # Stop worker
pm2 restart memsmith-worker # Restart worker
pm2 delete memsmith-worker  # Remove from PM2
pm2 logs memsmith-worker    # View logs
```
...
- **Logs**: `~/.memsmith/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.memsmith/.worker.pid`
- **Port File**: `~/.memsmith/.worker.port` (new)
...
- Database file: `~/.memsmith/memsmith.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'memsmith-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.memsmith/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.memsmith/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete memsmith-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.memsmith/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete memsmith-worker` (old worker terminated)
5. **Marker creation**: `~/.memsmith/.pm2-migrated` with timestamp
...
| `pm2 stop memsmith-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart memsmith-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete memsmith-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs memsmith-worker` | `npm run worker:logs` | View logs |
| `pm2 describe memsmith-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/memsmith-worker-out.log
     ~/.pm2/logs/memsmith-worker-error.log

New: ~/.memsmith/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/memsmith-worker.pid

New: ~/.memsmith/.worker.pid
```
...
New: ~/.memsmith/.worker.pid
     ~/.memsmith/.worker.port
     ~/.memsmith/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.memsmith/memsmith.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ memsmith-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove memsmith logs
rm -f ~/.pm2/logs/memsmith-worker-*.log
rm -f ~/.pm2/pids/memsmith-worker.pid
```
...
```
~/.memsmith/
├── memsmith.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── memsmith-worker-out.log
│   └── memsmith-worker-error.log
├── pids/
│   └── memsmith-worker.pid
└── pm2.log
```
...
```
~/.memsmith/
├── memsmith.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `memsmith-worker`
...
**Manual Resolution**:
```bash
rm ~/.memsmith/.worker.pid
rm ~/.memsmith/.worker.port
...
| `Invalid port X` | Port validation failed | Update `MEMSMITH_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name memsmith-worker
...
# 5. Verify migration
pm2 list  # Should NOT show memsmith-worker
cat ~/.memsmith/.pm2-migrated  # Should exist
