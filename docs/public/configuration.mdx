---
title: "Configuration"
description: "Environment variables and settings for MemSmith"
---
...
Settings are managed in `~/.memsmith/settings.json`. The file is auto-created with defaults on first run.

### Core Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `MEMSMITH_MODEL`            | `claude-haiku-4-5-20251001`     | Claude model used to compress observations (when using the Claude provider) |
| `MEMSMITH_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openrouter` |
| `MEMSMITH_CLAUDE_AUTH_METHOD` | `subscription`                | Claude provider auth mode: `subscription`, `api-key`, or `gateway` |
| `MEMSMITH_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `MEMSMITH_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `MEMSMITH_WORKER_PORT`      | `37700 + (uid % 100)`           | Worker service port (per-user default; override for fixed port) |
| `MEMSMITH_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `MEMSMITH_DATA_DIR`         | `~/.memsmith`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
| `MEMSMITH_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Gemini Provider Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `MEMSMITH_GEMINI_API_KEY`   | —                               | Gemini API key ([get free key](https://aistudio.google.com/app/apikey)) |
| `MEMSMITH_GEMINI_MODEL`     | `gemini-2.5-flash-lite`          | Gemini model: `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` |
...
| Setting                                      | Default                     | Description                           |
|----------------------------------------------|-----------------------------|---------------------------------------|
| `MEMSMITH_OPENROUTER_API_KEY`              | —                           | OpenRouter API key ([get key](https://openrouter.ai/keys)) |
| `MEMSMITH_OPENROUTER_MODEL`                | `xiaomi/mimo-v2-flash:free` | Model identifier (supports 100+ models) |
| `MEMSMITH_OPENROUTER_SITE_URL`             | —                           | Optional: URL for analytics           |
| `MEMSMITH_OPENROUTER_APP_NAME`             | `memsmith`                | Optional: App name for analytics      |
...
Gateway credentials live in `~/.memsmith/.env`, not `settings.json`.
...
Use [LiteLLM Gateway](configuration/litellm-gateway) when you want `MEMSMITH_PROVIDER=claude` to route through LiteLLM while preserving the Claude Agent SDK worker path.

### System Configuration

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `MEMSMITH_DATA_DIR`         | `~/.memsmith`                 | Data directory location               |
| `MEMSMITH_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
| `MEMSMITH_PYTHON_VERSION`   | `3.13`                          | Python version for chroma-mcp         |
| `CLAUDE_CODE_PATH`            | _(auto-detect)_                 | Path to Claude Code CLI (for Windows) |

## Model Configuration

Configure which Claude model compresses your observations (only applies when `MEMSMITH_PROVIDER=claude`).
...
`npx memsmith install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.memsmith/settings.json`.
...
| Setting | Default | Description |
|---------|---------|-------------|
| `MEMSMITH_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "MEMSMITH_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.memsmith/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `MEMSMITH_DATA_DIR`

```
~/.memsmith/
├── memsmith.db           # SQLite database
├── .install-version        # Version marker written by `npx memsmith install`/`repair`
...
MemSmith provides MCP search tools for querying your project history.
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.memsmith/settings.json`)
- **Auto-Invocation**: Claude recognizes natural language queries about past work

## Version Channel

MemSmith supports switching between stable and beta versions via the web viewer UI.

### Accessing Version Channel

1. Open the viewer at the worker URL (default `http://127.0.0.1:<worker-port>`; the active port is the value of `MEMSMITH_WORKER_PORT` in `~/.memsmith/settings.json`)
...
| Setting | Default | Description |
|---------|---------|-------------|
| `MEMSMITH_FOLDER_CLAUDEMD_ENABLED` | `false` | Enable auto-generation of folder CLAUDE.md files |
...
MemSmith injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
| **Worker Port** | `37700 + (uid % 100)` | Port for background worker service (override with `MEMSMITH_WORKER_PORT`) |
...
Settings are stored in `~/.memsmith/settings.json`:

```json
{
  "MEMSMITH_CONTEXT_OBSERVATIONS": "100",
  "MEMSMITH_CONTEXT_SESSION_COUNT": "20",
  "MEMSMITH_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "MEMSMITH_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "MEMSMITH_CONTEXT_FULL_COUNT": "10",
  ...
  "MEMSMITH_CONTEXT_SHOW_LAST_SUMMARY": "false",
...
Settings can be customized in `~/.memsmith/settings.json`.
...
Then restart the worker:
```bash
export MEMSMITH_MODEL=opus
npm run worker:restart
```

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.memsmith/settings.json`:
```json
{
  "MEMSMITH_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx memsmith install` / `npx memsmith repair` outside the session lifecycle.
...
If you specify an invalid Claude model name, the worker logs a warning and uses the default. Valid Claude models for `MEMSMITH_MODEL`:
...
1. Set custom port:
   ```bash
   export MEMSMITH_WORKER_PORT=38000
   ```
...
3. Verify new port:
   ```bash
   curl -s http://127.0.0.1:$MEMSMITH_WORKER_PORT/api/health | jq .port
   ```
