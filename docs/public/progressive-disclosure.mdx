# Progressive Disclosure: MemSmith's Context Priming Philosophy
...
## MemSmith's Solution: Progressive Disclosure
...
```markdown
*Use memsmith MCP search to access records with the given ID*
```
...
MemSmith implements progressive disclosure through a 3-layer workflow pattern:
...
- [MemSmith Architecture](architecture/overview) - How it all fits together
...
*This philosophy emerged from real-world usage of MemSmith across hundreds of coding sessions. The pattern works because it aligns with both human cognition and LLM attention mechanics.*
