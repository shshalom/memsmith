...
<p align="center">
  MemSmith бесшовно сохраняет контекст между сеансами, автоматически фиксируя наблюдения за использованием инструментов, генерируя семантические сводки и делая их доступными для будущих сеансов. Это позволяет Claude поддерживать непрерывность знаний о проектах даже после завершения или переподключения сеансов.
</p>
...
📚 **[Просмотреть полную документацию](https://docs.memsmith.ai/)** - Просмотр на официальном сайте

### Начало работы

- **[Руководство по установке](https://docs.memsmith.ai/installation)** - Быстрый старт и продвинутая установка
- **[Руководство по использованию](https://docs.memsmith.ai/usage/getting-started)** - Как MemSmith работает автоматически
- **[Инструменты поиска](https://docs.memsmith.ai/usage/search-tools)** - Запросы к истории проекта на естественном языке
- **[Бета-функции](https://docs.memsmith.ai/beta-features)** - Попробуйте экспериментальные функции, такие как режим Endless

### Лучшие практики

- **[Инженерия контекста](https://docs.memsmith.ai/context-engineering)** - Принципы оптимизации контекста для AI-агентов
- **[Прогрессивное раскрытие](https://docs.memsmith.ai/progressive-disclosure)** - Философия стратегии подготовки контекста в MemSmith

### Архитектура

- **[Обзор](https://docs.memsmith.ai/architecture/overview)** - Компоненты системы и поток данных
- **[Эволюция архитектуры](https://docs.memsmith.ai/architecture-evolution)** - Путь от v3 к v5
- **[Архитектура хуков](https://docs.memsmith.ai/hooks-architecture)** - Как MemSmith использует хуки жизненного цикла
- **[Справочник по хукам](https://docs.memsmith.ai/architecture/hooks)** - Объяснение 7 скриптов хуков
- **[Сервис Worker](https://docs.memsmith.ai/architecture/worker-service)** - HTTP API и управление Bun
- **[База данных](https://docs.memsmith.ai/architecture/database)** - Схема SQLite и поиск FTS5
- **[Архитектура поиска](https://docs.memsmith.ai/architecture/search-architecture)** - Гибридный поиск с векторной базой данных Chroma

### Конфигурация и разработка

- **[Конфигурация](https://docs.memsmith.ai/configuration)** - Переменные окружения и настройки
- **[Разработка](https://docs.memsmith.ai/development)** - Сборка, тестирование, участие в разработке
- **[Устранение неполадок](https://docs.memsmith.ai/troubleshooting)** - Распространенные проблемы и решения
...
Подробности см. в [Обзоре архитектуры](https://docs.memsmith.ai/architecture/overview).

---

## Навык mem-search

MemSmith предоставляет интеллектуальный поиск через навык mem-search, который автоматически вызывается, когда вы спрашиваете о прошлой работе:
...
Подробные примеры см. в [Руководстве по инструментам поиска](https://docs.memsmith.ai/usage/search-tools).

---

## Бета-функции

MemSmith предлагает **бета-канал** с экспериментальными функциями, такими как **режим Endless** (биомиметическая архитектура памяти для расширенных сеансов). Переключайтесь между стабильной и бета-версиями из веб-интерфейса на http://localhost:37777 → Settings.

Подробности о режиме Endless и способах его опробовать см. в **[Документации по бета-функциям](https://docs.memsmith.ai/beta-features)**.
...
Настройки управляются в `~/.memsmith/settings.json` (автоматически создается с настройками по умолчанию при первом запуске). Настройте AI-модель, порт worker, директорию данных, уровень логирования и параметры внедрения контекста.

Все доступные настройки и примеры см. в **[Руководстве по конфигурации](https://docs.memsmith.ai/configuration)**.

---

## Разработка

Инструкции по сборке, тестированию и процессу участия в разработке см. в **[Руководстве по разработке](https://docs.memsmith.ai/development)**.
...
Распространенные проблемы и решения см. в **[Руководстве по устранению неполадок](https://docs.memsmith.ai/troubleshooting)**.
...
Процесс участия см. в [Руководстве по разработке](https://docs.memsmith.ai/development).
...
- **Проблемы**: [GitHub Issues](https://github.com/shshalom/memsmith/issues)
- **Репозиторий**: [github.com/shshalom/memsmith](https://github.com/shshalom/memsmith)
- **Автор**: Alex Newman ([@shshalom](https://github.com/shshalom))
