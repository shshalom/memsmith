...
describe('MCP Integrations', () => {
  ...
  describe('writeMcpJsonConfig', () => {
    ...
    it('writes valid JSON with memsmith entry', () => {
      ...
      expect(config.mcpServers['memsmith'].command).toBe(process.execPath);
      expect(config.mcpServers['memsmith'].args).toEqual(['/path/to/mcp.cjs']);
    });
    ...
    it('uses custom serversKeyName when provided', () => {
      ...
      expect(config.servers['memsmith']).toBeDefined();
      expect(config.mcpServers).toBeUndefined();
    });

    it('preserves existing servers when adding memsmith', () => {
      const configPath = join(tempDir, 'mcp.json');
  ...
  describe('idempotency', () => {
    it('running install twice does not create duplicate entries', () => {
      ...
      const memSmithEntries = serverKeys.filter((k) => k === 'memsmith');
      expect(memSmithEntries).toHaveLength(1);
    });
    ...
    it('updates the server path on re-install', () => {
      ...
      const config = JSON.parse(readFileSync(configPath, 'utf-8'));
      expect(config.mcpServers['memsmith'].args).toEqual(['/new/path/mcp.cjs']);
    });
  });
