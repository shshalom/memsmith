...
describe('Codex CLI installer config repair', () => {
  it('adds memsmith plugin enablement when missing', () => {
    const result = setTomlPluginEnabled('model = "gpt-5.5"\n', 'memsmith@memsmith-local', true);

    expect(result).toContain('[plugins."memsmith@memsmith-local"]');
    expect(result).toContain('enabled = true');
  });

  it('updates existing plugin enablement in place', () => {
    const input = [
      '[plugins."memsmith@shshalom"]',
      'enabled = true',
      '',
      '[marketplaces.memsmith-local]',
      'source_type = "git"',
      '',
    ].join('\n');

    const result = setTomlPluginEnabled(input, 'memsmith@shshalom', false);

    expect(result).toContain('[plugins."memsmith@shshalom"]\nenabled = false');
    expect(result).toContain('[marketplaces.memsmith-local]');
  });

  it('inserts enabled into an existing plugin section without touching the next section', () => {
    const input = [
      '[plugins."memsmith@memsmith-local"]',
      '',
      '[hooks.state]',
      '',
    ].join('\n');

    const result = setTomlPluginEnabled(input, 'memsmith@memsmith-local', true);
  ...
  it('removes stale legacy memsmith mcp-search config', () => {
    const input = [
      'model = "gpt-5.5"',
      '',
      ...
      'command = "node"',
      'args = ["/Users/alexnewman/.codex/plugins/cache/memsmith-local/memsmith/12.7.5/scripts/mcp-server.cjs"]',
      '',
      '[plugins."memsmith@memsmith-local"]',
      'enabled = true',
      '',
  ...
  it('removes child tables for the stale legacy mcp-search config', () => {
    const input = [
      '[mcp_servers.mcp-search]',
      'command = "node"',
      'args = ["/tmp/memsmith/scripts/mcp-server.cjs"]',
      '',
  ...
  it('preserves non-memsmith mcp-search config', () => {
    const input = [
      '[mcp_servers.mcp-search]',
      'command = "python"',
      'args = ["server.py"]',
