...
describe('MemoryRoutes — POST /api/memory/save (#2116)', () => {
  ...
  beforeEach(() => {
    ...
    routes = new MemoryRoutes(mockDbManager as any, 'memsmith');
  });
  ...
  it('falls back to defaultProject when no project supplied anywhere', () => {
    ...
    expect(mockGetOrCreateManualSession).toHaveBeenCalledWith('memsmith');
    expect(storeObservationCalls[0][1]).toBe('memsmith');
  });
