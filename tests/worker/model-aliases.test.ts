...
/**
 * #2289 — $TIER alias syntax in MEMSMITH_MODEL resolves at request time to a
...
describe('resolveTierAlias (#2289)', () => {
  it('resolves $TIER:fast to MEMSMITH_TIER_FAST_MODEL', () => {
    const settings = settingsWith({ MEMSMITH_TIER_FAST_MODEL: 'claude-haiku-4-5-20251001' });
    expect(resolveTierAlias('$TIER:fast', settings)).toBe('claude-haiku-4-5-20251001');
  });

  it('resolves $TIER:smart to MEMSMITH_TIER_SMART_MODEL', () => {
    const settings = settingsWith({ MEMSMITH_TIER_SMART_MODEL: 'claude-sonnet-4-5-20250101' });
    expect(resolveTierAlias('$TIER:smart', settings)).toBe('claude-sonnet-4-5-20250101');
  });

  it('resolves $TIER:simple to MEMSMITH_TIER_SIMPLE_MODEL', () => {
    const settings = settingsWith({ MEMSMITH_TIER_SIMPLE_MODEL: 'haiku' });
    expect(resolveTierAlias('$TIER:simple', settings)).toBe('haiku');
  });

  it('resolves $TIER:summary to MEMSMITH_MODEL when no summary model is set', () => {
    const settings = settingsWith({ MEMSMITH_MODEL: 'claude-opus', MEMSMITH_TIER_SUMMARY_MODEL: '' });
  ...
  it('does not mutate the settings object', () => {
    const settings = settingsWith({ MEMSMITH_TIER_FAST_MODEL: 'haiku' });
  ...
  it('falls back to portable defaults when the tier model is empty', () => {
    const settings = settingsWith({ MEMSMITH_TIER_FAST_MODEL: '', MEMSMITH_TIER_SMART_MODEL: '' });
