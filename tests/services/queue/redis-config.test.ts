...
describe('redis queue config', () => {
  ...
  test('loads queue settings from settings file with env override precedence', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'memsmith-redis-config-'));
    const settingsPath = join(tempDir, 'settings.json');
    writeFileSync(settingsPath, JSON.stringify({
      MEMSMITH_QUEUE_ENGINE: 'bullmq',
      MEMSMITH_REDIS_MODE: 'external',
      MEMSMITH_REDIS_HOST: 'settings-host',
      MEMSMITH_REDIS_PORT: '6381',
      MEMSMITH_REDIS_URL: '',
      MEMSMITH_QUEUE_REDIS_PREFIX: 'settings-prefix',
    }), 'utf-8');
    ...
    setEnv('MEMSMITH_REDIS_HOST', 'env-host');
