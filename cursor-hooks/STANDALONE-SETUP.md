# MemSmith for Cursor (No Claude Code Required)

> **Persistent AI Memory for Cursor - Zero Cost to Start**

## Overview

Use memsmith's persistent memory in Cursor without a Claude Code subscription. Choose between free-tier providers (Gemini, OpenRouter) or paid options.
...
Since you don't have Claude Code, you need to configure an AI provider for memsmith's summarization engine.
...
cat > ~/.memsmith/settings.json << 'EOF'
{
  "MEMSMITH_PROVIDER": "gemini",
  "MEMSMITH_GEMINI_API_KEY": "YOUR_GEMINI_API_KEY",
...
cat > ~/.memsmith/settings.json << 'EOF'
{
  "MEMSMITH_PROVIDER": "openrouter",
  "MEMSMITH_OPENROUTER_API_KEY": "YOUR_OPENROUTER_API_KEY"
}
EOF
```
...
- Visit https://docs.memsmith.ai for full documentation
...
$settingsDir = "$env:USERPROFILE\.memsmith"
