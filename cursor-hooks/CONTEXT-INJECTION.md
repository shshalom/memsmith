...
1. **Install**: `memsmith cursor install` creates initial context file
...
3. Fetches existing context from memsmith and writes to `.cursor/rules/memsmith-context.mdc`
...
[Your context from memsmith appears here]
...
When you run `memsmith cursor install`, the project is registered in `~/.memsmith/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.memsmith/cursor-projects.json
```
...
| Initial setup | Automatic | `memsmith cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `memsmith cursor install`:
...
Configure memsmith's MCP server in Cursor for search tools:
...
Ask the agent: "Check memsmith for any previous work on authentication"
