...
- **By recency** (common) — worktrees active in a window:
  ```bash
  node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" worktrees --since <1h|4h|24h|7d|all> --json
  ```
...
- **By hand** — specific branches and/or open PRs:
  ```bash
  node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" worktrees --json   # local branches
  node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" prs --json         # open PRs (via gh)
  ```
...
```bash
node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" open --force --agent facilitator \
...
> `~/.memsmith/STANDUP.md`; speak with `node "<skill-dir>/ms-standup.mjs" post …`,
...
```bash
node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" summation --agent facilitator \
...
```bash
node "${CLAUDE_SKILL_DIR}/ms-standup.mjs" <command> [--flags]
```
