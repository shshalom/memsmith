...
try {
  let dataDir = process.env.MEMSMITH_DATA_DIR || join(homedir(), ".memsmith");
  if (!process.env.MEMSMITH_DATA_DIR) {
    const settingsPath = join(dataDir, "settings.json");
    if (existsSync(settingsPath)) {
      try {
        const settings = JSON.parse(readFileSync(settingsPath, "utf-8"));
        if (settings.MEMSMITH_DATA_DIR) dataDir = settings.MEMSMITH_DATA_DIR;
      } catch { /* use default */ }
    }
  }

  const dbPath = join(dataDir, "memsmith.db");
