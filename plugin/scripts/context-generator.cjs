...
`)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}setErrorSink(e){ne=e}error(e,s,t,r){this.log(3,e,s,t,r),this.routeErrorToSink(s,t,r)}routeErrorToSink(e,s,t){try{if(!ne||!(t instanceof Error))return;ne(t)}catch{}}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}},_=new ie;var Oe=require("crypto");function be(n,e,s){return(0,Oe.createHash)("sha256").update([n||"",e||"",s||""].join("\0")).digest("hex").slice(0,16)}var p="claude";function Js(n){return n.trim().toLowerCase().replace(/\s+/g,"-")}function M(n){if(!n)return p;let e=Js(n);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:p}function he(n){let e=["claude","codex","cursor"];return[...n].sort((s,t)=>{let r=e.indexOf(s),o=e.indexOf(t);return r!==-1||o!==-1?r===-1?1:o===-1?-1:r-o:s.localeCompare(t)})}function Ne(n,e,s,t,r){let o=Date.now()-t,i=r!==void 0?"up.session_db_id = ?":"up.content_session_id = ?",a=r??e;return n.prepare(`
  ...
  `).get(a,s,o)??void 0}var Ce=["private","memsmith-context","system_instruction","system-instruction","persisted-output","system-reminder"],Me=new RegExp(`<(${Ce.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g"),Le=/<system-reminder>[\s\S]*?<\/system-reminder>/g,Ae=100;function Qs(n){let e=Object.fromEntries(Ce.map(r=>[r,0]));Me.lastIndex=0;let s=0,t=n.replace(Me,(r,o)=>(e[o]=(e[o]??0)+1,s+=1,""));return s>Ae&&_.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:s,maxAllowed:Ae,contentLength:n.length}),{stripped:t.trim(),counts:e}}function ve(n){return Qs(n).stripped}var zs=["task-notification"],yt=new RegExp(`^\\s*<(${zs.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),Dt=256*1024;var ae=4e3;function B(n){let e=n.trim(),t=ve(n).trim()||e;return t.length<=ae?t:(_.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:t.length,storedLength:ae}),`${t.slice(0,ae-1)}\u2026`)}var j=class{db;constructor(e=ge){e instanceof de.Database?this.db=e:(e!==":memory:"&&Ie(N),this.db=new de.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA journal_size_limit = 4194304")),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(s=>s.name)}hasUniqueIndexOnColumns(e,s){return this.db.query(`PRAGMA index_list(${e})`).all().some(r=>{if(r.unique!==1)return!1;let o=this.getIndexColumns(r.name);return o.length===s.length&&o.every((i,a)=>i===s[a])})}resolvePromptSessionDbId(e,s,t){if(s!==void 0)return s;let r=t?M(t):void 0;return r?this.db.prepare(`
    ...
    `).all(...l);if(!a)return O;let S=new Map(O.map(A=>[A.id,A])),g=e.map(A=>S.get(A)).filter(A=>!!A);return r?g.slice(0,r):g}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:o,platformSource:i}=s,a=t==="relevance",d=a?"":`ORDER BY up.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,E=r?`LIMIT ${r}`:"",m=e.map(()=>"?").join(","),l=[...e],c=[];o&&(c.push("s.project = ?"),l.push(o)),i&&(c.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),l.push(M(i)));let T=c.length>0?`AND ${c.join(" AND ")}`:"",O=this.db.prepare(`
      ...
      ${d}
      ${E}
    `).all(...l);if(!a)return O;let S=new Map(O.map(g=>[g.id,g]));return e.map(g=>S.get(g)).filter(g=>!!g)}getTimelineAroundTimestamp(e,s=10,t=10,r,o){return this.getTimelineAroundObservation(null,e,s,t,r,o)}getTimelineAroundObservation(e,s,t=10,r=10,o,i){let a=i?M(i):void 0,d=(f,h)=>{let R=[],C=[];return o&&(R.push(`${f}.project = ?`),C.push(o)),a&&(R.push(`COALESCE(NULLIF(${h}.platform_source, ''), '${p}') = ?`),C.push(a)),{clause:R.length>0?`AND ${R.join(" AND ")}`:"",params:C}},E=d("o","src"),m=d("ss","src"),l=d("s","s"),c,T;if(e!==null){let f=`
