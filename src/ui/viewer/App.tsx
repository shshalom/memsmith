...
export function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    // Probe once: a 200 means the worker log endpoint exists; 404 (local/server
    // runtime) means hide the console.
    fetch('/api/logs')
      .then(r => { if (!cancelled) setConsoleAvailable(r.ok); })
      .catch(() => { if (!cancelled) setConsoleAvailable(false); });
