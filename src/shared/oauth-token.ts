...
async function readWindowsCredentialManager(): Promise<OAuthTokenResult> {
  ...
  const psScript = `
    ...
    Add-Type -Namespace MemSmith -Name CredRead -MemberDefinition @"
...
"@ -ErrorAction SilentlyContinue
    foreach ($t in $candidates) {
      $ptr = [IntPtr]::Zero
      $ok = [MemSmith.CredRead]::CredRead($t, 1, 0, [ref]$ptr)
      if ($ok) {
        $cred = [System.Runtime.InteropServices.Marshal]::PtrToStructure($ptr, [Type][MemSmith.CredRead+CREDENTIAL])
        $bytes = New-Object byte[] $cred.CredentialBlobSize
        [System.Runtime.InteropServices.Marshal]::Copy($cred.CredentialBlob, $bytes, 0, $cred.CredentialBlobSize)
        [MemSmith.CredRead]::CredFree($ptr) | Out-Null
