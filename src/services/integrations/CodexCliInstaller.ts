...
const MARKETPLACE_NAME = 'memsmith-local';
const CODEX_PLUGIN_ID = `memsmith@${MARKETPLACE_NAME}`;
const LEGACY_CODEX_PLUGIN_IDS = ['memsmith@shshalom'];
...
function resolvePluginMarketplaceRoot(preferredRoot?: string): string {
  ...
  throw new Error('Could not locate a Codex marketplace root with .agents/plugins/marketplace.json and plugin/.codex-plugin/plugin.json. Run npx memsmith@latest install from the package or repo root.');
}
...
function isMemSmithMcpSearchBlock(block: string): boolean {
  return /memsmith/.test(block);
}
...
export function removeLegacyCodexMcpSearchConfig(content: string): string {
  ...
  const removeLegacyMcpSearch = blocks.some(
    (block) => isLegacyMcpSearchHeader(block.header) && isMemSmithMcpSearchBlock(block.text),
  );
  ...
  // The stale memsmith-owned server can have tool child tables; remove the
...
function assertCodexMarketplaceSupported(): void {
  ...
  if (version.localeCompare(MIN_CODEX_MARKETPLACE_VERSION, undefined, { numeric: true }) < 0) {
    throw new Error(`Codex CLI ${version} is too old for plugin marketplace support. Update Codex CLI to ${MIN_CODEX_MARKETPLACE_VERSION} or newer, then run: npx memsmith@latest install`);
  }
}
...
export async function installCodexCli(marketplaceRootOverride?: string): Promise<number> {
  console.log('\nInstalling MemSmith for Codex CLI (native hooks)...\n');

  if (!commandExists('codex')) {
    console.error('Codex CLI was not found on PATH.');
    console.error('Install Codex, then run: npx memsmith@latest install');
    return 1;
  }
...
function performCodexInstall(marketplaceRootOverride?: string): number {
  ...
  runCodexBestEffort(
    ...
    'Could not refresh Codex marketplace cache; reinstall or upgrade memsmith from /plugins if Codex still uses old MCP config',
  );
...
For a fresh setup, the supported entry point is:
  npx memsmith@latest install
`);
  return 0;
}

export function uninstallCodexCli(): number {
  console.log('\nUninstalling MemSmith Codex CLI integration...\n');
