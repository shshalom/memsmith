...
import { OllamaProvider, isOllamaSelected, isOllamaAvailable } from '../../OllamaProvider.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private async startGeneratorWithProvider(
    ...
    const agent = provider === 'ollama' ? this.ollamaAgent : (provider === 'openrouter' ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'ollama' ? 'Ollama' : (provider === 'openrouter' ? 'OpenRouter' : (provider === 'gemini' ? 'Gemini' : 'Claude SDK'));
  ...
  private async applyTierRouting(session: NonNullable<ReturnType<typeof this.sessionManager.getSession>>): Promise<void> {
    ...
    if (settings.MEMSMITH_TIER_ROUTING_ENABLED === 'false') {
    ...
    if (hasSummarize) {
      const summaryModel = settings.MEMSMITH_TIER_SUMMARY_MODEL;
      if (summaryModel) {
      ...
      const simpleModel = settings.MEMSMITH_TIER_SIMPLE_MODEL;
      if (simpleModel) {
