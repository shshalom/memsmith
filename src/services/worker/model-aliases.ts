 ...
 *
 * Lets users write a portable tier alias in MEMSMITH_MODEL (e.g.
...
export function resolveTierAlias(model: string, settings: SettingsDefaults): string {
  ...
  switch (match[1]) {
    case 'fast':
      return settings.MEMSMITH_TIER_FAST_MODEL || 'haiku';
    case 'smart':
      return settings.MEMSMITH_TIER_SMART_MODEL || 'sonnet';
    case 'simple':
      return settings.MEMSMITH_TIER_SIMPLE_MODEL || 'haiku';
    case 'summary':
      ...
      return settings.MEMSMITH_TIER_SUMMARY_MODEL || settings.MEMSMITH_MODEL;
    default:
      return model;
  }
}
