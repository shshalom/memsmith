...
function resolveDbPath(): string {
  const dataDir = process.env.MEMSMITH_DATA_DIR || join(homedir(), '.memsmith');
  return join(dataDir, 'memsmith.db');
}
...
Notes:
  Operates directly on ~/.memsmith/memsmith.db (or \$MEMSMITH_DATA_DIR).
...
`);
  ...
  console.log(clearAll
    ? '\n=== MemSmith Queue Clearer (ALL) ===\n'
    : '\n=== MemSmith Queue Clearer (Processing) ===\n');
