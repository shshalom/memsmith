...
const DB_PATH = path.join(os.homedir(), '.memsmith', 'memsmith.db');
const SETTINGS_PATH = path.join(os.homedir(), '.memsmith', 'settings.json');
const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
const OBSERVATION_LIMIT = parseInt(settings.MEMSMITH_CONTEXT_OBSERVATIONS, 10) || 50;
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<memsmith-context>')) {
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      const stripped = content.replace(/<memsmith-context>[\s\S]*?<\/memsmith-context>/g, '').trim();
