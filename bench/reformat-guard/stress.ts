...
// --- The event corpus: varied, realistic agent activity. Each entry becomes a
// single generation context (one agent_event). Kept diverse on purpose so the
// measured format rate reflects real-world input variety, not one easy shape.
const EVENT_PAYLOADS: Array<{ label: string; eventType: string; payload: unknown }> = [
...
function makeContext(payload: unknown, eventType: string): ServerGenerationContext {
  ...
  return { job, events: [event], project: { projectId, teamId, serverSessionId: null, projectName: 'stress' } };
}
